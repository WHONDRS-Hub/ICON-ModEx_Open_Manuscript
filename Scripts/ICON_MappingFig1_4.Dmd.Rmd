...
ggplot(usa) +
  ...
  ylab("Latitude") + 
  xlab("Longitude") + 
   annotation_scale(location = "bl", width_hint = 0.3,  pad_x = unit(0.5, "in"), pad_y = unit(0.5, "in")) +
    annotation_north_arrow(location = "bl", which_north = "true", 
        pad_x = unit(0.75, "in"), pad_y = unit(0.75, "in"),
        style = north_arrow_fancy_orienteering) +
  ggtitle("Sites sampled across CONUS", subtitle = "Some subtitle here") +
  theme_bw() + 
      theme(panel.grid.major = element_line(color = gray(0.9), linetype = "dashed", 
        size = 0.1), panel.background = element_rect(fill = "aliceblue"), text = element_text(size = 18)) +
...
combods %>% filter(Observed_Normalized_Respiration_Rate_mg_DO_per_H_per_L_sediment < abs(500)) %>% #filter not working
  ggplot(aes(abs(Predicted_Normalized_Respiration_Rate_mg_DO_per_H_per_L_sediment), abs(Observed_Normalized_Respiration_Rate_mg_DO_per_H_per_L_sediment))) +
  geom_point(size = 4, alpha = 0.5) + 
    geom_abline (slope=1, linetype = "dashed", color="Red") + 
#    stat_poly_line(color = "light grey", alpha = 0.01) +
 # stat_poly_eq(use_label(c( "R2"))) +
...
Obs <- ggplot(usa) +
  ...
  geom_spatial_point(data=combods, aes(x=Longitude,y=Latitude, fill = log(-LogObs+10)), pch = 21, size =4) +     
 ...
 theme_bw() +
   theme(panel.background = element_rect(fill = "aliceblue"), text = element_text(size = 15), legend.position ="none") +
    labs(fill = "log(Resp. + 10)") 
 ...
 Pred<- 
  ...
  geom_sf(color = "#2b2b2b", fill = "#FCFCFC", size=0.125) +
  coord_sf(crs = st_crs("+proj=laea +lat_0=45 +lon_0=-100 +x_0=0 +y_0=0 +a=6370997 +b=6370997 +units=m +no_defs"), datum = NA) +
  geom_spatial_point(data=combods, aes(x=Longitude,y=Latitude, fill = log(-LogPred+10)), pch = 21, size =4) +     
  #scale_shape_manual(values = c(16, 8)) +
   # scale_size_manual(values = c(4, 8)) +
  scale_fill_distiller( palette = "Oranges", direction = 1, 
                        limits = c(min(log(0+10)), max(log(4000+10)) ) )  + 
  ...
  xlab("Longitude")  +
   ggtitle("Predicted respiration rates at sampled sites") +
        annotation_scale(location = "br", width_hint = 0.3,  pad_x = unit(0.1, "in"), pad_y = unit(0.1, "in")) +
  theme_bw() +
   theme(panel.background = element_rect(fill = "aliceblue"), text = element_text(size = 15), legend.position = "bottom") +
       labs(fill = "log(Resp. + 10)") 
