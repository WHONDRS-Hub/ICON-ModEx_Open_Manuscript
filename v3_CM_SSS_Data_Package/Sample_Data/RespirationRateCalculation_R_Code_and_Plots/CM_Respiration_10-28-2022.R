...
library(dplyr); library(ggplot2);library(ggsignif); library(tidyverse)
...
#Example:
pnnl.user = 'gara009'
input.path = paste0("C:/Users/",pnnl.user,"/OneDrive - PNNL/Documents - Core Richland and Sequim Lab-Field Team/Data Generation and Files/RC4/ICON_ModEX/02_Raw_DO_INC_data/")
...
data = read_csv("CM_DO_INC.xlsx") #reads all the data from all treatments
...
if (nrow(pts.removed)>1){
  pts.removed = pts.removed[-1,]
        ggplot(data_site_subset, aes(x = time_elapsed_min, y = AVG_DO_mg_per_L)) + coord_cartesian(ylim = c(ymin,ymax))+ geom_point(size = 2) + expand_limits(x = 0, y = 0) + 
          ...
          stat_poly_eq(formula = my.formula,label.y = "bottom",label.x = "left", aes(label = paste( ..rr.label.., sep = "~~~"),size=1), parse = TRUE)+stat_fit_glance(data=data_site_subset, method = 'lm', method.args = list(formula = my.formula),geom = 'text',aes(label =paste("p = ",signif(..p.value.., digits = 1), sep = ""),size=1),label.y = c(11.75),label.x = c(3)) +
          ...
          geom_text(aes(label = paste0("p = ",round(pog,5)), y = ymax, x = data_collection_time_min-6),color = "red")+
        ...
        ggsave(file=paste0(output.path,"Plots/DO_vs_Incubation_Time_",data_site_subset$Sample_label[1],".pdf"))}else{
          ggplot(data_site_subset, aes(x = time_elapsed_min, y = AVG_DO_mg_per_L)) + coord_cartesian(ylim = c(ymin,ymax))+ geom_point(size = 2) + expand_limits(x = 0, y = 0) + 
            geom_smooth(method = "lm", se=F, formula = my.formula, color = "black") + geom_text(aes(label = paste0("R2 = ",round(rfinal,3)), y = 0, x = c(3)),color = "black")+
            geom_text(aes(label = paste0("p = ",round(pfinal,5)), y = ymax, x = c(3)),color = "black")  + geom_text(aes(label = paste0("R2 = ",round(rog,3)), y = 0, x = data_collection_time_min-6),color = "red")+
            geom_text(aes(label = paste0("p = ",round(pog,5)), y = ymax, x = data_collection_time_min-6),color = "red")+
            theme_bw()+theme(legend.title = element_blank(),legend.background = element_rect(fill = 'NA'), legend.text = element_text(size = 12,face="bold"))+
            labs(y = expression(Dissolved_Oxygen_mg_per_L), x = expression(Time_Elapsed_min))+ theme(axis.text.x=element_text(size = 12,face="bold"))+
            ggtitle(data_site_subset$Sample_label[1]) + 
...
#Exports data
write.csv(respiration.metrics,paste0(output.path,"ICON-ModEx_Sediment_Incubations_Respiration_Rates_merged_by_",pnnl.user,"_on_",Sys.Date(),".csv"), row.names = F)
