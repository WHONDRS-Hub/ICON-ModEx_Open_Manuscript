...
# Print unique iteration dates and numerical IDs
unique(combined_df$Date)
unique(combined_df$iteration_num)


# Subset 2nd iteration
second_iter_df <- combined_df %>% filter(Date == '2021-12-01')


# /----------------------------------------------------------------------------#
#/   PRINT MEAN AND SD.DEV. FOR FIRST AND SECOND ITERATIONS.
...
iter2019_df <- combined_df %>% filter(iteration_num == 1)
iter2023_11 <- combined_df %>% filter(iteration_num == 19)
